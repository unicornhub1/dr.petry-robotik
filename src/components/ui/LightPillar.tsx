...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  const qualitySettings = {
    low: { iterations: 50, pixelRatio: 1, targetFPS: 30 },
    medium: { iterations: 70, pixelRatio: Math.min(1.5, window.devicePixelRatio), targetFPS: 45 },
    high: { iterations: 100, pixelRatio: Math.min(2, window.devicePixelRatio), targetFPS: 60 },
  }
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Calculate alpha based on color brightness (luminance)
        // Dark areas become transparent, bright areas stay visible
        vec3 finalColor = color * uIntensity;
        float luminance = dot(finalColor, vec3(0.299, 0.587, 0.114));
        float alpha = smoothstep(0.0, 0.12, luminance);
