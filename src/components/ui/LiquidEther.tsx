...
const defaultColors = ['#da4e24', '#ff6b3d', '#0098f3']
...
export default function LiquidEther({
  ...
  const qualityPresets = {
    low: { iterations: 16, resolution: 0.35, pixelRatio: 1 },
    medium: { iterations: 24, resolution: 0.5, pixelRatio: Math.min(1.5, window.devicePixelRatio || 1) },
    high: { iterations: 32, resolution: 0.5, pixelRatio: Math.min(2, window.devicePixelRatio || 1) },
  }
  const preset = qualityPresets[quality]

  // Use prop values if provided, otherwise use quality presets
  const finalIterationsViscous = iterationsViscous ?? preset.iterations
  const finalIterationsPoisson = iterationsPoisson ?? preset.iterations
  const finalResolution = resolution ?? preset.resolution
...
`
    ...
    class ExternalForce extends ShaderPass {
      ...
      initMouse(simProps: Record<string, unknown>) {
        super.init()
      ...
      update(props?: Record<string, unknown>) {
        props = props || {}
        const forceX = (Mouse.diff.x / 2) * ((props.mouse_force as number) || 0)
        const forceY = (Mouse.diff.y / 2) * ((props.mouse_force as number) || 0)
        const cellScale = (props.cellScale as THREE.Vector2) || { x: 1, y: 1 }
        const cursorSize = (props.cursor_size as number) || 0
  ...
  return (
    <div
      ref={mountRef}
      className={`w-full h-full relative overflow-hidden pointer-events-auto touch-none ${className || ''}`}
      style={style}
    />
  )
}
