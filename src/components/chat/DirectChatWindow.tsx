...
export default function DirectChatWindow({ recipientId }: DirectChatWindowProps) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    const unread = messages.filter(
      (m) => m.sender_id === recipientId && !m.admin_read_at
    )
  ...
  const handleFileUpload = async (file: File) => {
    ...
    const supabase = createClient()
    const filePath = `messages/direct/${recipientId}/${Date.now()}_${file.name}`
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header (desktop only, mobile has its own in parent) */}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-0">
        ...
        {messages.map((msg) => (
          <ChatBubble
            ...
            senderName={msg.sender_id === user?.id ? 'Sie' : 'Kunde'}
